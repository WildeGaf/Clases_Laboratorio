...
/** \brief BUSCA UN ID DE CLIENTE EN EL ARRAY CLIENTES.
* \param Clientes arrayClientes[] ES EL ARRAY DONDE BUSCA EL ID.
* \param int longitudArray tamaño del array de CLIENTES.
* \param int codigoCliente el id del CLIENTE a buscar.
...
*/
 ...
 /** \brief ORDENA EL ARRAY DE CLIENTES POR INSERCION EN ORDEN DESCENDENTE POR APELLIDO.
* \param Clientes arrayClientes[] ES EL ARRAY A ORDENAR.
* \param int longitudArray tamaño del array de CLIENTES.
* \return VOID
*
*/
     ...
     if (arrayClientes!= NULL && longitudArray > 0)
     {
        for(i=1;i<longitudArray;i++)
        {
            if (arrayClientes[i].isEmpty != 1)
            {
                ...
                while(j>=0 && (strcmp(auxArrayClientes.apellido,arrayClientes[j].apellido) >0)) // temp<data[j] o temp>data[j]
                {
...
* \return contadorAltasClientes PARA INDICAR SI SE REALIZO EL ALTA O NO DE UN CLIENTE.
*
*/

int menuClientes(Clientes arrayClientes [], int CANT_CLIENTES, int contadorAltasClientes)
{
    char auxApellidoCliente[51], auxNombreCliente[51], auxDomicilio[51], auxTelefono[21];
    int opcionesClientes, contadorClientes=0, auxCodigoCliente, opcionesDosClientes, auxIndexClientes, verificarIdClientes, index, respuesta;
