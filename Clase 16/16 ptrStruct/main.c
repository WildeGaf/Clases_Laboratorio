...
int main()
{
    ...
    printf("\nNOMBRE: ");
    setbuf(stdin, NULL);
    scanf("%[^\n]", &per.nombre); //%[^\n] Acepta todo menos lo q esta entre corchetes
    ...
    printf("\nNOMBRE: ");
    setbuf(stdin, NULL); //Igual que el fflush pero se usa en cualquier SO
    scanf("%[^\n]", ptrPer->nombre);

    printf("FECHA NACIMIENTO\n");

    printf("\nDIA: ");
    scanf("%d", &ptrPer->fechaNac.dia); //La flecha solo se usa del puntero al campo.
    ...
    for(i=0; i<2; i++){
        ...
        printf("\nDIA: ");
        scanf("%d", &(*(ptrPers+i)).fechaNac.dia);// una estructura dentro de otra
